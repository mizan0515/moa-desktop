...
- [ ] 첫 화면 = workbench (랜딩 페이지 X). 왼쪽 세션 list, 가운데 작업 입력 + 진행 상태, 오른쪽 결과 영역, 아래 로그 영역 (모두 더미 데이터 OK)
...
- [ ] `src-tauri/src/` 아래 다음 빈 module folder + mod.rs 미리 생성: process, adapters, safety, git, lock, journal, telemetry, cancel, mock, orchestrator, synthesis, settings — 각 mod.rs 는 `// owned by T<n>` 주석만
- [ ] `Cargo.toml` 에 공통 deps 미리 등록: tokio (full), serde, serde_json, anyhow, thiserror, tauri-plugin-single-instance
- [ ] SynthesisView.tsx, ClaimLedger.tsx, CostMeter.tsx, ErrorBanner.tsx 는 stub (placeholder div) 로 placeholder
...
- `src/components/{SynthesisView,ClaimLedger,CostMeter,ErrorBanner}.tsx` (stub only)
- `src-tauri/Cargo.toml`, `src-tauri/tauri.conf.json`, `src-tauri/build.rs`
- `src-tauri/src/main.rs`, `src-tauri/src/lib.rs`
- `src-tauri/src/{process,adapters,safety,git,lock,journal,telemetry,cancel,mock,orchestrator,synthesis,settings}/mod.rs` (빈 stub)
