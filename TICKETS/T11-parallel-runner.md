...
T11 lane scheduler 는 T10 의 `runtimeKind: "claude" | "codex" | "pi"` 를 소비한다. Pi lane 은 T15b/T15c capability 가 available 인 경우에만 활성화하고, 초기 permission 은 read-only/research/reviewer/conversational 이다. Pi mutation owner 는 T15g 이후 opt-in setting 과 T4 mutation lock/T15d trust/T15e UI gate/mandatory review gate all-pass 전까지 queued/blocked 로 표시한다.
...
7. Pi lane 은 `PiRpcAdapter`/`PiSdkHost` capability manifest, package trust state, extension UI capability 를 lane admission 시 검증한다. `pi install`, `pi update`, package hot reload 는 scheduler 가 직접 실행하지 않고 blocked/needs-confirm state 로만 표시한다.
...
- [ ] lane result schema 에 `{ticketId, runtimeKind, runtimePermission, runtimeCapabilities, status, patchPath, phaseGuide, conflictMatrixRef, dependencyGraphRef, mergeOrderRef, github, reviewGateRequired, reviewProfileId, reviewScope, reviewRunRecords}` 포함. `reviewRunRecords[]` 는 T13 L2.5 의 full audit schema (`verdict`, `reviewer`, `review_kind`, `review_profile_id`, `reasoning_effort`, `model_or_profile_id`, `prompt_template_version`, `prompt_hash`, `command_source_adapter`, `primary_role`, `scope`, `gate`, `patch_hash`, `files_reviewed`, `omitted_files`, `limitations`, `evidence`, `required_actions`, `created_at`, `source_output_path`) 를 그대로 보존한다. worker output 또는 command_guard 에 peer AI 직접 호출 흔적이 있으면 failed lane 으로 표시하고 T12 에 전달하지 않음. `runtimeKind="pi"` 의 advisory review 는 mandatory `CodexAdversarialXHigh` gate 를 대체하지 않는다.
...
1. Success criteria: N lane worktree pool, lock ordering, resource budget, lane result schema, full `ReviewRunRecord` audit field persistence, resume packet 복원을 구현한다.
...
3. Validation cmd:
   ...
   ```
4. Files + lines: `TICKETS/T11-parallel-runner.md` 의 Success criteria/Constraints, `TICKETS/T10-ticket-decomposer.md` 의 schema, `TICKETS/T12-merge-integrator.md` 의 lane result 소비 조건, `TICKETS/T13-policy-lifecycle-epic.md` L2.5 의 ReviewVerdict/ReviewInputStrategy/ReviewRunRecord schema.
...
- Dependency/merge order: T10 의 `phaseGuide`, `dependencyGraph`, `mergeOrder`, `conflictMatrix`, `runtimeKind` 를 lane scheduler 입력으로 보존한다. T12 는 T11 산출물만 소비하므로 T11 완료 전 T12 시작 금지.
