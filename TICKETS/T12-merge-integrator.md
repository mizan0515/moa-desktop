# T12 — Merge Integrator + Pi lane records (`/병행통합` 등가)
...
7. T11 의 Pi lane result 도 `runtimeKind`, `ReviewRunRecord`, patch metadata, omitted files, limitations 를 동일하게 검증한다. Pi lane 은 T15g opt-in 전 patch artifact 가 있으면 policy violation 으로 stop 한다.
...
- [ ] Pi lane validation: `runtimeKind="pi"` lane 은 `piSessionId`, `piExtensionPolicyRef`, `piPackageManifestHash`, limitations 를 검증한다. T15g opt-in 전 `patchPath` 또는 mutation metadata 가 있으면 merge 하지 않고 `ReviewRunError` + 한국어 보고로 stop 한다.
...
- Pi lane result 는 Claude/Codex lane result 와 같은 gate/persistence 검증을 통과해야 한다.
- Pi review 는 additional signal 로만 저장한다. mandatory `CodexAdversarialXHigh` gate 는 그대로 필요하다.
- Pi package install/update/hot reload audit record 가 lane result 에 있으면 T15d/e policy record 와 hash 를 대조한다.
- Pi mutation owner 는 T15g opt-in 전 merge 대상이 아니다. patchPath 가 있으면 stop 하고 "Pi lane mutation is disabled before T15g opt-in" 으로 보고한다.
- Pi session tree/ResumePacket mirror 는 restore 보조자료이며 merge order source of truth 는 T10 `mergeOrder` 이다.

## Worker prompt 6 mandatory fields
1. Success criteria: T10 `mergeOrder` 순서대로 T11 lane patch 를 apply/check/rollback 하고, conflict stop/report, cleanup, review gate verdict matrix, runtimeKind 별 lane validation, full ReviewRunRecord persistence 를 구현한다.
...
3. Validation cmd:
   ...
   ```
4. Files + lines: `TICKETS/T12-merge-integrator.md` 의 Goal/Success criteria/Constraints/T15 amend, `TICKETS/T10-ticket-decomposer.md` 의 `mergeOrder`/`reviewGate`, `TICKETS/T11-parallel-runner.md` 의 lane result schema, `TICKETS/T13-policy-lifecycle-epic.md` L2.5 의 ReviewInputStrategy/ReviewRunRecord.
...
- Dependency/merge order: T10 `mergeOrder` 를 source of truth 로 사용하고, T11 lane result 의 `runtimeKind`, `dependencyGraphRef`, `conflictMatrixRef`, `reviewRunRecords` 를 검증한 뒤 적용한다. T11 완료 전 T12 시작 금지.
