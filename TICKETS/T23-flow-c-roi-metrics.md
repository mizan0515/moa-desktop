...
## Codex cost fallback 전략
현재 `src-tauri/src/telemetry/cost.rs` 에서 Codex cost 가 $0 으로 기록되는 문제 해결 방안:
1. **1차 시도**: Codex CLI stdout/stderr 에서 `usage` JSON 필드 파싱 (`prompt_tokens`, `completion_tokens`, `total_tokens`). Codex CLI 의 `--json` 모드에서 `item.completed` event 의 usage 필드 확인.
2. **2차 fallback**: usage 필드가 없으면 `completion_tokens ≈ output_bytes / 4` + `prompt_tokens ≈ input_bytes / 4` 로 character-based estimation. 모델별 price table (`codex-mini-latest`: $0.0015/1K input, $0.006/1K output) 적용.
3. **3차 fallback**: 모든 estimation 실패 시 `cost_codex: null` + `cost_estimation_method: "unavailable"` 로 기록. dashboard 에서 Codex cost 열을 "추정 불가" 표시.
4. **v1.0 이전 테스트**: mock outcome record 20건을 fixture 로 생성하여 dashboard 동작 검증. 실사용 데이터 없이도 통계 계산·렌더링 경로 테스트 가능.

## Risks
1. **Codex cost=$0 문제**: OpenAI Codex CLI가 usage 응답을 제공하지 않으면 cost 추적 불가능. **대응**: 위 fallback 전략 적용
